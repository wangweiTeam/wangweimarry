...
<div class="page-order-comfirm" id="orderComfirm">
    <div class="section-con" v-if="addrIsShow">
        ...
        <div class="addressAddBox" >
            <form action="/address?method=addAddress" model="formItem" ref="formItem" method="post">
                <div class="addr-add-form">
                    <div class="div-form-gw">
                        <label class="label-gw"><span class="mark">*</span>收货人：</label>
                        <input type="text" placeholder="" required class="input-gw input-gw220" v-model="formItem.addr_person" name="username">
                    </div>
                    <div class="div-form-gw">
                        ...
                        <div class="div-three clearfix">
                            <div class="div-three-item">
                                <select name="province" class="select-gw province" v-model="formItem.addr_province" >
                                    <option v-for="(provinceOne,index) in provinceList" >
                                        {{provinceOne.value}}
                                    </option>
                                </select>
                            </div>
                            <div class="div-three-item">
                                <select name="city" class="select-gw city" v-model="formItem.addr_city">
                                    <option v-for="(cityOne,index) in cityList" value="" >
                                        {{cityOne.value}}
                                    </option>
                                </select>
                            </div>
                            <div class="div-three-item div-right">
                                <select  name="district" class="select-gw district" v-model="formItem.addr_district">
                                    <option v-for="(districtOne,index) in districtList" value="" >
                                        {{districtOne.value}}
                                    </option>
                                </select>
                            </div>
                    ...
                    <div class="div-form-gw">
                        ...
                        <input type="text" placeholder="" class="input-gw input-gw480" v-model="formItem.addr_moreD" name="detailaddress"/>
                    </div>
                    <div class="div-form-gw">
                        <div class="div-half clearfix">
                            <div class="div-half-item">
                                <label class="label-gw"><span class="mark">*</span>手机号码：</label>
                                <input type="tel" placeholder="" required  maxlength="11" class="input-gw input-gw220 input-cur-gw" v-model="formItem.addr_tel" name="phone"/>
                            </div>
                            <div class="div-half-item div-right">
                                <label class="label-gw">固定电话</label>
                                <input type="text" placeholder="" class="input-gw input-gw220 input-error-gw" v-model="formItem.addr_ftel" name="gdphone"/>
                            </div>
                        </div>
                    </div>
                    <div class="div-form-gw">
                        <label class="label-gw">邮箱：</label>
                        <input type="email" placeholder="" required class="input-gw input-gw480" v-model="formItem.addr_email" name="email"/>
                    </div>
                    <div class="addr-sumbit-gw">
                        <input type="submit" value="保存收货地址" class="btn-gray-gw" />
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="section-con">
        <div class="sec-title-border clearfix">
            ...
            <a href="personalcenter.jsp" class="right">返回个人中心</a>
        </div>
        <div class="addressBox clearfix">
            <div class="address-item" v-for="(item,index) in addressList" :key="index" :class="{'selected':item.selected}" @click="selecteAddr(index)">
                <c:forEach items="${sessionScope.useraddresses}" var="useraddresses">
                    <div class="address-item-in">
                        <div>收件人：${useraddress.getUsername()}</div>
                        <div>地址：${useraddress.getDetail()}</div>
                        <div>电话：${useraddress.getGdPhone()}</div>
    ...
    new Vue({
        ...
        methods:{
            ...
            add(){
                ...
                var atpos=this.formItem.addr_email.indexOf("@");
                var dotpos=this.formItem.addr_email.lastIndexOf(".");
